...
# ==============================================================================
# 2. ENHANCED SIMULATION & CALCULATION ENGINE
# ==============================================================================
@st.cache_data
def get_processed_data(fuel_hike_percent=6.0, holiday_spike_percent=25.0):
    # A. Generate Simulated raw scraped data
    np.random.seed(42)
    random.seed(42)
        ...
        for route in routes:
            ...
            for lead in lead_times:
                ...
                for _ in range(5):
                    ...
                    if random.random() < 0.015:  # Random scraper errors / extreme spikes
    ...
    # B. Clean outliers using segmentation IQR
    cleaned_frames = []
    ...
    df_cleaned = pd.concat(cleaned_frames).reset_index(drop=True)

    # C. Calculate APIx (DGCA-weighted Laspeyres Index)
    ...
    final_index_df = apix_overall.merge(apix_t1, on='date').merge(apix_t45, on='date')
    final_index_df['Weekly_APIx'] = final_index_df['APIx'].rolling(window=7, min_periods=1).mean()
...
# Execute Data Engine with custom parameters
raw_data, clean_data, apix_df = get_processed_data(sim_fuel, sim_holiday)

# Apply Sidebar filtering selections
